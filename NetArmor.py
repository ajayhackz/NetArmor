...
def get_ip_details(ip_address, port):
    ...
    if response.status_code == 200:
        ...
        # Return the response text directly if it's not a 200 status
        return response.text  # This will be a string, not a dictionary


def print_colored_details(address, ip_details, malicious):
    print(f"Details for {Fore.CYAN}{address[0]}:{address[1]}{Style.RESET_ALL}:")
    
    if isinstance(ip_details, dict):  # Check if ip_details is a dictionary
        print(f"   IP: {Fore.GREEN}{ip_details.get('ip', 'N/A')}{Style.RESET_ALL}")
        print(f"   Location: {Fore.YELLOW}{ip_details.get('city', 'N/A')}, {ip_details.get('region', 'N/A')}, {ip_details.get('country', 'N/A')}{Style.RESET_ALL}")
        print(f"   ISP: {Fore.MAGENTA}{ip_details.get('org', 'N/A')}{Style.RESET_ALL}")
        ...
        if malicious is not None:
            ...
            print(Fore.YELLOW + "   Malicious: Unknown (Error querying VirusTotal)" + Style.RESET_ALL)
    else:
        # If ip_details is not a dictionary, print the error message
        print(Fore.RED + f"   Error retrieving IP details: {ip_details}" + Style.RESET_ALL)
